        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          PageHead(
            ...
            action: _tab == 'bills'
                ...
                : _tab == 'schedules' && isLeaseholder
                    ? ElevatedButton(onPressed: () => _openScheduleModal(context, state, null), child: const Text('+ Schedule'))
                    : null,
          ),
          Segment<String>(
            options: const ['bills', 'schedules', 'history'],
            value: _tab,
            labelFor: (v) => v == 'bills'
                ? 'Upcoming'
                : v == 'schedules'
                    ? 'Schedules${schedules.isNotEmpty ? ' · ${schedules.length}' : ''}'
                    : 'History${history.isNotEmpty ? ' · ${history.length}' : ''}',
          ...
          if (_tab == 'bills') ...[
            if (upcoming.isEmpty)
              const EmptyState(title: 'No upcoming bills', body: 'Paid-off bills move to History. Add one when the next invoice arrives.'),
            for (final b in upcoming) _BillCard(bill: b, canManage: isLeaseholder, onEdit: () => _openBillModal(context, state, b)),
...
class _ShareRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pay = bill.payments[userId];
    final isYou = userId == cu.id;
    final canMark = isYou || cu.role == 'leaseholder';
            ...
            if (paid && pay != null)
              Padding(
                ...
                child: Row(children: [
                  ...
                  Flexible(
                    child: Text(
                      'Paid by ${pay.payerName} · ${fmtDateShort(pay.at)}'
                      '${pay.confirmedBy != null ? ' · confirmed by ${pay.confirmedBy}' : ''}',
  ...
  Future<void> _markPaid(BuildContext context, HomiesState state, User user) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'Mark ${isYou ? 'your' : "${user.name}'s"} share of ${fmtAUD(amount)} as paid?\n\n'
          'This is saved under ${user.name}\'s name with today\'s date'
    ...
    if (ok != true) return;
    state.mutate(() {
      bill.paidBy[userId] = true;
      bill.payments[userId] = Payment(
        payerId: userId,
        payerName: user.name,
      ...
      bill.status = bill.shares.keys.every((uid) => bill.paidBy[uid] == true) ? 'settled' : 'pending';
    });
  }
  ...
  Future<void> _undo(BuildContext context, HomiesState state) async {
    ...
    if (ok != true) return;
    state.mutate(() {
      bill.paidBy[userId] = false;
      bill.payments.remove(userId);
      bill.status = 'pending';
    });
  }
}
