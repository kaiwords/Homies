...
import 'marketplace.dart' show goodsCatIcon;
...
/// 1:1 chat between a buyer and seller, scoped to a GoodsListing. Mirrors
/// EssentialThreadScreen's structure with a goods-listing context bar instead
/// of a business one.
class MarketplaceThreadScreen extends StatefulWidget {
  final GoodsListing listing;
  final String otherUserId;
  const MarketplaceThreadScreen({super.key, required this.listing, required this.otherUserId});
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(children: [
          _GoodsContextBar(listing: widget.listing),
          Expanded(
...
class _GoodsContextBar extends StatelessWidget {
  final GoodsListing listing;
  const _GoodsContextBar({required this.listing});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        Icon(goodsCatIcon(listing.category), size: 16, color: HomiesColors.textDim),
        const SizedBox(width: 8),
        Expanded(
          child: Text('${listing.title} · \$${listing.price.toStringAsFixed(listing.price % 1 == 0 ? 0 : 2)}',
