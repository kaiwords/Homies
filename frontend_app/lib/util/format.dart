...
final _time = DateFormat('h:mm a', 'en_AU');
...
/// Clock time for a chat message, e.g. "3:42 pm".
String fmtTime(String? iso) {
  final d = parseIso(iso);
  if (d == null) return '';
  return _time.format(d).toLowerCase();
}

/// A day separator label for a chat timeline: "Today", "Yesterday", or a date.
String fmtChatDay(String? iso) {
  final d = parseIso(iso);
  if (d == null) return '';
...
/// Compact mm:ss for a media duration given in milliseconds.
