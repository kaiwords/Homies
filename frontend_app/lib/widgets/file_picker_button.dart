...
class FilePickerButton extends StatefulWidget {
  ...
  final String? label;
  const FilePickerButton({super.key, required this.value, required this.onChanged, this.allowedExtensions, this.label});
  ...
  Future<void> _pick() async {
    ...
    try {
      ...
      if (f.size > 2 * 1024 * 1024) {
        setState(() {
          _busy = false;
          _error = 'File is ${(f.size / 1024 / 1024).toStringAsFixed(1)} MB — keep it under 2 MB for the demo.';
        });
        return;
      }
      final bytes = f.bytes;
  ...
  Widget build(BuildContext context) {
    if (widget.value != null) {
      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(
          ...
          child: TextButton(onPressed: () => widget.onChanged(null), child: const Text('Replace')),
        ),
      ]);
    }
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      if (_error != null)
        Padding(
          padding: const EdgeInsets.only(top: 6),
          child: Text(_error!, style: const TextStyle(color: HomiesColors.danger, fontSize: 12)),
        ),
    ]);
  }
}
